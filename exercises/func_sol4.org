...
#+BEGIN_SRC haskell
  ...
  -- terminate on a finit list whose sum of all the elements less than n, for
  -- example, sumWhile 7 [1, 1, 1].
  
  sumListCond :: Int -> Int -> [Int] -> Int
  sumListCond l n xs | foldl (+) 0 (take l xs) <= n
                       && take (l - 1) xs /= take l xs = sumListCond (l + 1) n xs
