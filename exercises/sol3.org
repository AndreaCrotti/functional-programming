...
* Exercise 1
** (a)
[EUS] It is spelled "Polynomial" not "Polynomials"    
#+BEGIN_SRC haskell
    data Polynomial a = Const a | Var String | Sum (Polynomial a) (Polynomial a) | Prod (Polynomial a) (Polynomial a)
#+END_SRC
...
** (e)
[EUS] It is spelled "Tropical" not "Tropicals"
      It is spelled "Infinity" not "Infinity"
#+BEGIN_SRC haskell
...
** (f)
[EUS] For the case of plus(x, inf) and plus(inf, x), x is defined as x <- T not x <- Z
      Let's just use "()" instead of "$" to make it readable for lay men ;-)
#+BEGIN_SRC haskell
...
** (g)
[EUS] (g) asks us to define Tropical an instance of Eq and Ord
#+BEGIN_SRC haskell
  instance Eq Tropical where (Finite x) == (Finite y) = x == y
                             Infinity == Infinity = True
                             _ == _ = False

  instance Ord Tropical where (Finite x) > (Finite y) = x > y
...
#+BEGIN_SRC haskell
  ...
  countABs t = (foldTree (\x y z -> 1 + y + z) (\x y z -> 0 + y + z) 0 t, foldTree (\x y z -> 0 + y + z) (\x y z -> 1 + y + z) 0 t)
#+END_SRC
